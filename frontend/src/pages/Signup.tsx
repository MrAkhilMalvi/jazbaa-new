      ...
      try {
        ...
        if (!json.error && json.data?.states) {
          const uniqueStates: any = [
            ...new Set(json.data.states.map((s: any) => s.name.trim())),
          ].sort();
      ...
      try {
        ...
        if (!json.error && json.data) {
          const uniqueCities: any = [
            ...new Set(json.data.map((city: string) => city.trim())),
          ].sort();
            ...
            >
                    ...
                    >
                      ...
                      <SelectContent>
                        {citiesList.map((ct, index) => (
                          <SelectItem key={`${ct}-${index}`} value={ct}>
                            {ct}
                          </SelectItem>
                        ))}
                      </SelectContent>
                  ...
                  {[
                    {
                      value: "Member",
                      label: "A Member",
                      d: "Attend events & connect",
                    },
                    {
                      value: "Volunteer",
                      label: "A Volunteer",
                      d: "Help organize & engage",
                    },
                    {
                      value: "Lead",
                      label: "A Lead",
                      d: "Lead a city Chapter",
                    },
                  ].map((o) => (
                    <button
                      key={o.value}
                      type="button"
                      onClick={() => setCategory(o.value as any)}
                      className={cn(
                ...
                <label className="flex items-start gap-3 cursor-pointer group">
                  <div className="relative flex items-center justify-center mt-0.5 shrink-0">
                    ...
                    <div className="w-5 h-5 rounded border-2 border-slate-300 dark:border-white/20 flex items-center justify-center transition-all peer-checked:border-[#ff6a3d] peer-checked:bg-[#ff6a3d]/10">
                      <Check
                        className={cn(
                          "w-3.5 h-3.5 text-[#ff6a3d] transition-all duration-200",
                          consent
              ...
              <div className="flex flex-col-reverse sm:flex-row gap-4 items-center justify-between pt-2 relative z-10">
                <p className="text-xs md:text-sm font-medium text-slate-500 dark:text-white/50 text-center sm:text-left">
                  ...
                  <Link
                    to="/login"
                    className="text-[#ff6a3d] hover:underline font-bold sm:ml-1"
                  >
                    Sign in here
                  </Link>
                </p>
